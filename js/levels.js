...
//   grid: { cols, rows },
//   cells: [{ col, row, type }],
//     types: 'try-line' | 'obstacle' | 'waypoint' | 'mud' | 'hurdle' | 'opponent-player' | 'teammate-player' | 'ball'
...
const GROUPS = [
  {
    ...
    levels: [
      ...
      },
      {
        ...
        description: "The ball is loose on the pitch! Run to it, Pick Up Ball, then race to the try line and score.",
        grid: { cols: 7, rows: 5 },
        cells: [
          { col: 3, row: 0, type: 'ball' },
          { col: 6, row: 4, type: 'try-line' },
        ],
        finn: { startCol: 0, startRow: 2, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'pickup-ball', 'score-try'],
        winCondition: {
        ...
        maxCommands: null,
        hint: 'Move to the ball, use Pick Up Ball! to grab it, then navigate to the try line and Score Try!',
      },
      {
        id: 'mv-5',
        groupId: 'movement',
    ...
    levels: [
      {
        id: 'cond-1',
        groupId: 'conditions',
        title: 'The Lurking Defender',
        description: "A defender hides in one of two spots — blocking your lane OR lurking beside it. Every time you press Run they pick a new hiding place! Use IF to check what's directly ahead.",
        grid: { cols: 5, rows: 3 },
        cells: [
          { col: 4, row: 1, type: 'try-line' },
        ],
        randomObstacles: [
          { col: 2, row: 1, type: 'obstacle' },
        ...
        winCondition: { type: 'reach', target: { col: 4, row: 1 } },
        maxCommands: 4,
        requiredBlocks: ['if-condition'],
        hint: 'Forward. IF obstacle ahead → TL, F, TR, F, F, TR, F, TL. ELSE → F, F. Then Forward, Score Try!',
      },
      {
        id: 'cond-2',
        groupId: 'conditions',
        title: 'Spot the Gap',
        description: "The defender shuffles between lanes every run — sometimes right in your path, sometimes beside it. Use IF/ELSE to handle both!",
        grid: { cols: 7, rows: 3 },
        cells: [
          { col: 6, row: 1, type: 'try-line' },
        ],
        ...
        maxCommands: 5,
        requiredBlocks: ['if-condition'],
        hint: 'Forward twice. IF obstacle ahead → TL, F, TR, F, F, F, TR, F, TL. ELSE → F, F, F. Then Forward, Score Try!',
      },
      {
        id: 'cond-3',
        groupId: 'conditions',
        title: 'Muddy Pitch',
        description: "It rained and mud appeared somewhere on the pitch — it might be in your lane or off to the side. Your IF block detects mud too! Dodge it or run straight through.",
        grid: { cols: 7, rows: 3 },
        cells: [
          { col: 6, row: 1, type: 'try-line' },
        ],
        randomObstacles: [
          { col: 3, row: 1, type: 'mud' },
          { col: 3, row: 2, type: 'mud' },
        ],
        finn: { startCol: 0, startRow: 1, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'if-condition', 'score-try'],
        winCondition: { type: 'reach', target: { col: 6, row: 1 } },
        maxCommands: 5,
        requiredBlocks: ['if-condition'],
        hint: 'Same plan — but now check for mud! Forward twice. IF mud ahead → take the top route. ELSE → push straight through. Then Forward, Score Try!',
      },
      {
        id: 'cond-4',
        groupId: 'conditions',
        title: 'Scout the Centre',
        description: "A big defender patrols the centre of the pitch — sometimes in your lane, sometimes below it. Write a program that handles both without knowing in advance!",
        grid: { cols: 9, rows: 5 },
        cells: [
          { col: 8, row: 2, type: 'try-line' },
        ],
        randomObstacles: [
          { col: 4, row: 2, type: 'obstacle' },
          { col: 4, row: 3, type: 'obstacle' },
        ],
        finn: { startCol: 0, startRow: 2, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'if-condition', 'score-try'],
        ...
        maxCommands: 6,
        requiredBlocks: ['if-condition'],
        hint: 'Forward 3 times. IF obstacle ahead → TL, F, TR, F×4, TR, F, TL. ELSE → F×4. Then Forward, Score Try!',
      },
      {
        id: 'cond-5',
        groupId: 'conditions',
        title: 'Pick Up and Dodge',
        description: "The ball is loose — grab it, then deal with a defender who could be in your lane or off to the side! You'll need IF/ELSE after the pick-up.",
        grid: { cols: 7, rows: 3 },
        cells: [
          { col: 1, row: 1, type: 'ball' },
          { col: 6, row: 1, type: 'try-line' },
        ],
        randomObstacles: [
          { col: 3, row: 1, type: 'obstacle' },
        ...
        finn: { startCol: 0, startRow: 1, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'pickup-ball', 'if-condition', 'score-try'],
        winCondition: {
          type: 'visit-waypoints-then-reach',
          waypoints: [{ col: 1, row: 1 }],
          target: { col: 6, row: 1 },
        },
        maxCommands: 6,
        requiredBlocks: ['if-condition'],
        hint: 'Forward, Pick Up Ball!, Forward. IF obstacle ahead → TL, F, TR, F, F, F, TR, F, TL. ELSE → F, F, F. Then Forward, Score Try!',
      },
      {
        id: 'cond-6',
        groupId: 'conditions',
        title: 'Championship Pressure',
        description: "The biggest game of the season! A defender guards the halfway line — they pick a new side every run. Your program must handle BOTH. This is what IF/ELSE is made for!",
        grid: { cols: 11, rows: 5 },
        cells: [
          { col: 10, row: 2, type: 'try-line' },
        ],
        randomObstacles: [
        ...
        finn: { startCol: 0, startRow: 2, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'if-condition', 'score-try'],
        winCondition: { type: 'reach', target: { col: 10, row: 2 } },
        maxCommands: 7,
        requiredBlocks: ['if-condition'],
        hint: 'Forward 4 times. IF obstacle ahead → TL, F, TR, F×5, TR, F, TL. ELSE → F×5. Then Forward, Score Try!',
      },
    ],
  },
...

  {
    ...
    levels: [
      ...
      },
      {
        ...
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'sprint', 'turn-around', 'pickup-ball', 'if-condition', 'repeat', 'while-clear', 'tackle', 'score-try'],
        winCondition: {
          type: 'visit-waypoints-then-reach',
      ...
      },
      {
        ...
        description: "A teammate is calling for the ball! Pass to them, then race to the try line and score!",
        grid: { cols: 8, rows: 5 },
        cells: [
          { col: 3, row: 1, type: 'obstacle' },
        ...
        finn: { startCol: 1, startRow: 3, startDir: 'east' },
        availableCommands: ['move-forward', 'turn-left', 'turn-right', 'pass', 'score-try'],
        winCondition: {
          type: 'visit-waypoints-then-reach',
          waypoints: [{ col: 5, row: 1 }],
          target: { col: 7, row: 3 },
        },
        maxCommands: null,
        hint: 'Move east to col 5, pass north with strength 2 to your teammate, then move east 2 more to the try line and Score Try!',
      },
    ],
  },
];
